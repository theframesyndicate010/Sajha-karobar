...
async function fetchSummary() {
    try {
        ...
        if (el('stock-alerts')) {
            const alerts = [];
            if (data.stock_value > 0) alerts.push(`<span class="text-[#1e40af]"><i class="fa-solid fa-coins"></i> Rs ${(data.stock_value || 0).toLocaleString()} value</span>`);
            if (data.out_of_stock_count > 0) alerts.push(`<span class="text-[#2563eb]"><i class="fa-solid fa-circle-exclamation"></i> ${data.out_of_stock_count} out of stock</span>`);
            if (data.low_stock_count > 0) alerts.push(`<span class="text-[#2563eb]"><i class="fa-solid fa-triangle-exclamation"></i> ${data.low_stock_count} low stock</span>`);
            if (alerts.length === 0) alerts.push('<span class="text-[#2563eb]"><i class="fa-solid fa-check-circle"></i> All stocked</span>');
        ...
        topProductsList.innerHTML = products.map((product, idx) => `
            <div class="flex items-center justify-between py-2.5 px-3 rounded-xl hover:bg-slate-50 transition group">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-slate-500 text-xs font-bold group-hover:from-[#1e40af] group-hover:to-[#2563eb] group-hover:text-white transition">
                        ${idx + 1}
                    </div>
                    <div>
                ...
                <p class="font-bold text-sm text-[#1e40af]">Rs ${(product.total_revenue || 0).toLocaleString()}</p>
            </div>
        `).join('');
    } catch (error) {
...
function renderProducts(products) {
    ...
    productTableBody.innerHTML = products.map(product => `
        <tr class="hover:bg-gray-50 transition">
            ...
            <td class="py-4 px-6">
                <span class="${(product.stock_quantity || product.quantity || 0) === 0 ? 'text-[#2563eb]' : (product.stock_quantity || product.quantity || 0) <= 10 ? 'text-[#2563eb]' : 'text-[#1e40af]'} font-medium">
        ...
        tbody.innerHTML = sales.slice(0, 10).map((sale, idx) => {
            ...
            return `
                <tr class="hover:bg-slate-50 transition group">
                    <td class="py-4 px-6 font-semibold text-[#2563eb]">#SALE-${String(sale.id).slice(-4).padStart(4, '0')}</td>
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-3">
                            <div class="h-8 w-8 rounded-full bg-gradient-to-br from-[#1e40af] to-[#2563eb] text-amber-400 flex items-center justify-center text-xs font-bold">${initials}</div>
                    ...
                    <td class="py-4 px-6 text-right">
                        <button class="text-slate-400 hover:text-[#1e40af] p-1 rounded-lg hover:bg-slate-100 transition"><i class="fa-solid fa-ellipsis-vertical"></i></button>
                    </td>
                </tr>
            `;
        }).join('');
