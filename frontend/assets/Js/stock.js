...
function renderProducts() {
            ...
            <tr>
                <td colspan="7" class="px-6 py-12 text-center">
                    <div class="flex flex-col items-center justify-center">
                        ...
                        <button onclick="openAddModal()" class="mt-4 inline-flex items-center gap-2 bg-[#0F766E] hover:bg-[#134E4A] text-white font-semibold py-2 px-4 rounded-lg transition">
    ...
    tbody.innerHTML = filteredProducts.map(product => {
        ...
        if (isOutOfStock) {
            statusBadge = `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-slate-100 text-[#134E4A]">Out of Stock</span>`;
            rowClass = 'bg-slate-50';
        } else if (isLowStock) {
            statusBadge = `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-slate-100 text-[#134E4A]">Low Stock</span>`;
            rowClass = 'bg-slate-50';
        } else {
            statusBadge = `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-slate-100 text-[#134E4A]">In Stock</span>`;
        }
        ...
        return `
            <tr class="hover:bg-gray-50 transition ${rowClass}">
                ...
                <td class="px-6 py-4 text-center">
                    <span class="font-semibold ${isOutOfStock ? 'text-[#0F766E]' : isLowStock ? 'text-[#0F766E]' : 'text-gray-800'}">${qty}</span>
                </td>
                <td class="px-6 py-4 text-center">${statusBadge}</td>
                <td class="px-6 py-4 text-center">
                    <div class="flex items-center justify-center gap-2">
                        <button onclick="editProduct('${product.id}')" class="p-2 text-[#0F766E] hover:bg-slate-50 rounded-lg transition" title="Edit">
                            <i class="fa-solid fa-pen-to-square"></i>
                        </button>
                        <button onclick="deleteProduct('${product.id}', '${escapeHtml(product.name)}')" class="p-2 text-[#0F766E] hover:bg-slate-50 rounded-lg transition" title="Delete">
...
function showAddMessage(messageDiv, message, type) {
    messageDiv.classList.remove('hidden', 'bg-slate-50', 'border-slate-300', 'text-[#134E4A]');
    
    if (type === 'success') {
        messageDiv.className = 'bg-slate-50 border border-slate-300 text-[#134E4A] px-4 py-3 rounded-lg text-sm';
