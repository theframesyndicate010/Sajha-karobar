    ...
    tbody.innerHTML = bills.map((bill, index) => `
        <tr class="hover:bg-gray-50 transition">
            ...
            <td class="px-6 py-4 text-center">
                <span class="px-3 py-1 rounded-full text-sm font-semibold ${bill.payment_type === 'full' ? 'bg-slate-100 text-[#134E4A]' : 'bg-slate-50 text-[#134E4A]'}">
                    ${bill.payment_type === 'full' ? 'Full' : 'Partial'}
                </span>
            </td>
            <td class="px-6 py-4 text-right text-gray-700">Rs ${(bill.paid_amount || 0).toFixed(2)}</td>
            <td class="px-6 py-4 text-right font-semibold ${(bill.remaining_balance || 0) > 0 ? 'text-[#0F766E]' : 'text-[#134E4A]'}">
                Rs ${(bill.remaining_balance || 0).toFixed(2)}
            </td>
            <td class="px-6 py-4 text-center">
                <div class="grid grid-cols-2 gap-1 w-32 mx-auto">
                    <button onclick="viewBillDetails(${index})" class="bg-[#0F766E] hover:bg-[#134E4A] text-white font-semibold py-0.5 px-1 rounded text-[10px] transition">
                        👁️ View
                    </button>
                    <button onclick="editBillPayment(${index})" class="bg-[#0F766E] hover:bg-[#134E4A] text-white font-semibold py-0.5 px-1 rounded text-[10px] transition">
                        ✏️ Edit
                    </button>
                    <button onclick="printBill(${index})" class="bg-[#0F766E] hover:bg-[#134E4A] text-white font-semibold py-0.5 px-1 rounded text-[10px] transition">
                        🖨️ Print
                    </button>
                    <button onclick="deleteBill(${index})" class="bg-[#0F766E] hover:bg-[#134E4A] text-white font-semibold py-0.5 px-1 rounded text-[10px] transition">
                        🗑️ Delete
                    </button>
                </div>
            </td>
...
async function viewBillDetails(index) {
        ...
        <div class="space-y-4">
            ...
            <div class="border-t pt-4 grid grid-cols-2 gap-4">
                ...
                <div class="border-t pt-2 col-span-2">
                    ...
                    <p class="font-bold text-lg text-[#0F766E]">Rs ${(bill.total_amount || 0).toFixed(2)}</p>
                </div>
            </div>
            ...
            <div class="border-t pt-4 grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="font-semibold text-gray-800 px-3 py-1 bg-slate-100 rounded w-fit">
                ...
                <div>
                    ...
                    <p class="font-semibold ${(bill.remaining_balance || 0) > 0 ? 'text-[#0F766E]' : 'text-[#134E4A]'}">
                        Rs ${(bill.remaining_balance || 0).toFixed(2)}
                    </p>
                </div>
            </div>
                ...
                <div class="space-y-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                    ${payments.map((payment, i) => `
                        <div class="flex justify-between items-center p-2 bg-white rounded border border-gray-100">
                            ...
                            <p class="font-bold text-[#0F766E]">Rs ${(payment.payment_amount || 0).toFixed(2)}</p>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    actionBtn.className = 'bg-[#0F766E] hover:bg-[#134E4A] text-white font-semibold py-2 px-4 rounded-lg transition';
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                .payment-badge { display: inline-block; padding: 5px 10px; background-color: ${paymentType === 'full' ? '#0F766E' : '#0F766E'}; color: white; border-radius: 3px; margin: 5px 0; }
        ...
        <body>
            ...
            <div class="totals">
                ...
                ${(bill.remaining_balance || 0) > 0 ? `<p style="color: #0F766E;"><strong>Balance:</strong> Rs ${(bill.remaining_balance || 0).toFixed(2)}</p>` : `<p style="color: #0F766E;"><strong>Status:</strong> Fully Paid</p>`}
            </div>
            ...
            <div class="border-t pt-4">
                ...
                <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg mb-4 border border-slate-300">
                    <div>
                        ...
                        <p class="font-bold text-lg text-[#0F766E]">Rs ${(bill.remaining_balance || 0).toFixed(2)}</p>
                    </div>
                    <div>
                ...
                <div class="bg-slate-50 p-4 rounded-lg border border-slate-300 mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Amount Given by Retailer (Rs) <span class="text-[#0F766E]">*</span></label>
                    <input type="number" id="newPendingBalanceInput" min="0" step="0.01" placeholder="Enter payment received" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent text-lg" value="0.00">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-slate-50 p-4 rounded-lg border border-slate-300">
                        ...
                        <p class="font-bold text-xl text-[#0F766E]" id="reductionAmountDisplay">Rs 0.00</p>
                    </div>
                    <div class="bg-slate-50 p-4 rounded-lg border border-slate-300">
                        <p class="text-sm font-semibold text-gray-700 mb-2">New Paid Amount</p>
                        <p class="font-bold text-xl text-[#0F766E]" id="newPaidAmountDisplay">Rs ${(bill.paid_amount || 0).toFixed(2)}</p>
                    </div>
                </div>
            </div>
        </div>
